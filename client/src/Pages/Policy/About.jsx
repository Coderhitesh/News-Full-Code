  ...
  return (
    <div className="container mx-auto mt-5">
      <div className="flex flex-wrap">
        {/* About Us Section */}
        <div className="w-full lg:w-2/3 about-us-container mb-8 lg:mb-0">
          <div className="text-center mb-4">
            <img src="https://i.ibb.co/4jMB0SG/image.png" alt="Logo" className="mx-auto mb-3" />
            <h1 className="text-4xl font-bold">About Us ( हमारे बारे में) </h1>
          </div>
        ...
        <div className="w-full lg:w-1/3 vertical-news">
          {news.slice(0, 2).map((item, index) => (
            <Link to={`/news-page/${item._id}`} className="block mb-4 news-item" key={index}>
              <div className="category-col-img">
                <img
                  ...
                  alt={item.headline}
                  className="w-full h-[17rem] rounded-lg shadow"
                />
              </div>
              <div className="mt-2">
                <div className="flex justify-between items-center text-sm">
                  <span className="bg-blue-600 text-white px-2 py-1 rounded">{item.newsCategory}</span>
                  <span className="text-gray-600">{new Date(item.createdAt).toLocaleString()}</span>
                </div>
                <p className="mt-2 font-semibold text-lg">{item.headline}</p>
              </div>
      ...
      <div className="text-center mt-8">
        ...
        <p className="mt-4">
          <i className="fa-solid fa-envelope"></i> <a href="mailto:contact@aamawaz.com" className="text-blue-600 hover:underline">contact@aamawaz.com</a>
        </p>
        <p className="mt-2">
