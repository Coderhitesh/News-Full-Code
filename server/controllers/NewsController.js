...
exports.toggleisPopular = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // Toggle the isNewsPopular field
    newsItem.isNewsPopular = !newsItem.isNewsPopular;
    ...
    console.error("Error toggling news popularity:", error);
    res.status(500).json({ message: "Server error" });
  }
};
